...
class Ray:
    ...
    def draw_ray(self):
        ax = plt.gca()
        plt.plot([self.position[0], self.position[0] + self.direction[0] * 0.1], [self.position[1], self.position[1] + self.direction[1] * 0.1])
    ...
    def castCircle(self, circle):
        ...
        i = circle.xy1[0] #x coordinate of the circle
        j = circle.xy1[1] #y coordinate of the circle
        if ray_x1 - ray_x2 != 0:
            ...
            a = (m ** 2) + 1
            b = -(2 * i) - (2 * m * j) + (2 * m * k)
            c = (i ** 2) + (j ** 2) + (k ** 2) - (circle.r ** 2) - (2 * k * j)
            ...
            a = 1
            b = -(2 * j)
            c = (i ** 2) + (j ** 2) + (k ** 2) - (circle.r ** 2) - (2 * k * j) + (k ** 2)
